...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    public function testFlushOrdering()
    {
        ...
        // 4) Add another route
        $routes->add('/static', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/static', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function providePrefixTests()
    {
        ...
        $tests[] = array('/{static}', '/foo', '/{static}/foo');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        // embedded collection under /static
        $adminBlogRoutes = $routes->createBuilder();
        $adminBlogRoutes->add('/new', 'BlogController::newAction', 'admin_blog_new');
        // mount into static, but before the parent collection has been mounted
        $adminRoutes->mount('/blog', $adminBlogRoutes);

        // now mount the /static routes, above should all still be /blog/static
        $routes->mount('/static', $adminRoutes);
        ...
        $loader
            ...
            ->method('load')
            ->will($this->returnValue($importedCollection));
        // import this from the /static route builder
        $adminRoutes->import('static.yml', '/imported');

        $collection = $routes->build();
        $this->assertEquals('/static/dashboard', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/static/users', $collection->get('admin_users')->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/static/blog/new', $collection->get('admin_blog_new')->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/static/stats/sales', $collection->get('admin_stats_sales')->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
        $this->assertEquals('/static/imported/foo', $collection->get('imported_route')->getPath(), 'Normal RouteCollections are also prefixed properly');
    }
